...
# NAT Gateways (one per AZ for high availability, or single for cost savings)
resource "aws_nat_gateway" "main" {
  count         = var.single_nat_gateway ? 1 : length(var.availability_zones)
  allocation_id = aws_eip.nat[count.index].id
  subnet_id     = aws_subnet.public[0].id  # Always use first public subnet
...
# Private Route Tables (one per AZ if multiple NAT gateways, otherwise single)
